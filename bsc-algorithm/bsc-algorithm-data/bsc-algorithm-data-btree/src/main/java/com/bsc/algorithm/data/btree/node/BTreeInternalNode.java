...
public class BTreeInternalNode<K extends Comparable<K>> extends AbstractBTreeNode<K> {
	...
	public BTreeInternalNode(int degree) {
		super(degree);
	...
	public void deleteNotEmpty(K key) {
		...
		if (this.existsKey(key)) {
			...
			if ((node = children[index]).nkey() >= degree) {
				K repKey = node.getKey(node.nkey() - 1); // maximum key in
															// predecessor
		...
		// key may exist in child
			...
			if (target.nkey() >= degree) {
				...
				if (i > 0 && (sibling = children[i - 1]).nkey() >= degree) {
					if (!target.isLeaf()) {
						AbstractBTreeNode<K> sub = sibling.deleteChild(nchild); // last
																				// child
						target.insertChild(sub, 0);
					}
					K repKey = sibling.deleteKey(sibling.nkey() - 1); // maximum
																		// key
				...
				// try to find replacement from follower
					...
					if (!target.isLeaf()) {
						AbstractBTreeNode<K> sub = sibling.deleteChild(0); // first
																			// child
	...
	public void splitChild(int child) {
		...
		AbstractBTreeNode<K> neo = old.isLeaf() ? new BTreeLeafNode<>(degree) : new BTreeInternalNode<>(degree);
