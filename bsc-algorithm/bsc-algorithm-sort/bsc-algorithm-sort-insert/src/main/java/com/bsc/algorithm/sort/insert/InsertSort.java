...
public class InsertSort<T extends Comparable<T>> extends AbstractSort<T> {
	...
	public void sort(T[] data, int cr, int firstIndex, int lastIndex) {
		sort(data, cr, 1, firstIndex, lastIndex);
	}
	 ...
	 * @param increment 步长
	 */
	protected void sort(T[] data,int cr, int increment, int firstIndex, int lastIndex) {
		int length = lastIndex + 1;
		// 从第increment个数开始，和左边的数(间隔为increment)比较(左边的数(间隔为increment)已经排好序)
		for (int i = increment + firstIndex; i < length; i += increment) {
			T current = data[i];			
			int j = i - increment;
			// 左边已排好的数与当前current比较，如果较大(升序),较小(降序),第j个数就向左移increment位
			while (j >= firstIndex && compare(data[j],current) == cr) {
